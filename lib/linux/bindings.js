...
BlenoBindings.prototype.sendLeConnParamUpdateReq = function (minInterval, maxInterval, slaveLatency, timeoutMultiplier) {

  var LE_CHANNEL_CID = 0x0005;

  var data = new Buffer(12);

  this._connParamUpdateReqIndex = this._connParamUpdateReqIndex ? this._connParamUpdateReqIndex + 1 : 1;

  data.writeUInt8(0x12, 0); //LE_CON_PARAM_UPDATE_REQ_CODE
  data.writeUInt8(0x01, this._connParamUpdateReqIndex); // Need to randomize? Sequential?

  data.writeUInt16LE(0x0008, 2); // LE_CON_PARAM_UPDATE_LEN
  data.writeUInt16LE(minInterval, 4); // Min. interval
  data.writeUInt16LE(maxInterval, 6); // Max. interval
  data.writeUInt16LE(slaveLatency, 8); // Slave latency
  data.writeUInt16LE(timeoutMultiplier, 10); // Timeout Multiplier

  this._hci.writeAclDataPkt(this._handle, LE_CHANNEL_CID, data);
};
